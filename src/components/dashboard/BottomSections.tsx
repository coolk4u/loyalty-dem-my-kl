...
const BottomSections = () => {
  ...
  return (
    <div className="px-6 space-y-8 pb-8">
      {/* Marketing Materials */}
      <Card className="glass-card-dark border-gray-200 backdrop-blur-lg p-6 shadow-enterprise">
        <h3 className="text-xl font-bold text-gray-900 mb-6">Marketing Materials</h3>
        <div className="grid grid-cols-3 gap-4">
          {marketingMaterials.map((material, index) => (
            <Button
              key={index}
              variant="outline"
              className="border-gray-300 glass-card-dark text-gray-900 hover:bg-gray-100 hover:text-gray-900 p-4 h-auto flex-col space-y-3 transition-all duration-300 hover:shadow-enterprise"
            >
              ...
              <span className="text-sm text-center text-gray-900 font-medium">{material.name}</span>
              <span className="text-xs text-yellow-600 font-semibold px-2 py-1 bg-yellow-400/10 rounded-full">{material.type}</span>
            </Button>
          ))}
        </div>
      </Card>
      ...
      <Card className="glass-card-dark border-gray-200 backdrop-blur-lg p-6 shadow-enterprise">
        <h3 className="text-xl font-bold text-gray-900 mb-6">Social Media Connect</h3>
        <div className="flex justify-center space-x-6">
          {socialMedia.map((social, index) => (
            <div
              key={index}
              className={`w-16 h-16 ${social.bgColor} backdrop-blur-sm rounded-xl flex items-center justify-center shadow-enterprise hover:shadow-enterprise-lg transition-all duration-300 cursor-pointer hover:scale-110 border border-gray-200`}
            >
      ...
      {/* Notification Feed */}
      <Card className="glass-card-dark border-gray-200 backdrop-blur-lg p-6 shadow-enterprise">
        <h3 className="text-xl font-bold text-gray-900 mb-6">Notification Feed</h3>
        <div className="space-y-4">
          {notifications.map((notification) => (
            <div key={notification.id} className="flex items-start space-x-4 p-4 glass-card-dark rounded-lg border border-gray-100 hover:border-gray-300 transition-all duration-300">
              <div className={`w-10 h-10 ${notification.bgColor} rounded-full flex items-center justify-center flex-shrink-0`}>
                <notification.icon className={`w-5 h-5 ${notification.color}`} />
              </div>
              <div className="flex-1">
                <h4 className="text-gray-900 font-semibold text-sm mb-1">{notification.title}</h4>
